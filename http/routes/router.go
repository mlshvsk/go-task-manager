...
func NewRouter() *mux.Router {
	...
	projects.Handle("", handlers.Handler(controllers.IndexProjects)).Queries("page", "{page:[0-9]+}", "limit", "{limit:[0-9]+}").Methods(http.MethodGet)
	...
	projects.Handle("/{projectId:[0-9]+}/columns", handlers.Handler(controllers.IndexColumns)).Queries("page", "{page:[0-9]+}", "limit", "{limit:[0-9]+}").Methods(http.MethodGet)
	...
	columns.Handle("/{columnId:[0-9]+}/position", handlers.Handler(controllers.MoveColumn)).Methods(http.MethodPut)
	...
	tasks.Handle("/{taskId:[0-9]+}/position", handlers.Handler(controllers.MoveTask)).Methods(http.MethodPut)
	tasks.Handle("/{taskId:[0-9]+}/column/{newColumnId:[0-9]+}", handlers.Handler(controllers.MoveTaskColumn)).Methods(http.MethodPut)
